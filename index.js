...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `salve @${num.split('@')[0]}\ bv ao grupo *${mdata.subject}* se apresenta e da uma mamadinha do arabe`
				...
				teks = `🤬ala mais um saiu e nao pagou a coca🤬 @${num.split('@')[0]} até quando isso ?`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
                ...
                case 'hidetag':
					...
					if (!isOwner) return reply('af kkkkk')
...
case 'lofi':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️ESCUTA KAYBLACK PO KK'})
					break
                 case 'logoph':
					var gh = body.slice(9)
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('SAFADO KKKK')
					reply(mess.wait)
                ...
                case 'logo3d':
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'N E POEMA NAO KKK', text, {quoted: mek})
			...
			case 'mapa': 
				...
				await client.client.leaveGroup(from, 'adeus...', groupId)
                    break
				case 'bc': 
					if (!isOwner) return reply(' achando q e o arabe kkk') 
           ...
           case 'rebaixar':
			...
			case 'remover':
				...
				case 'boasvindas':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('agora vou falar bv af')
						welkom.push(from)
